var $mobx = mobx.$mobx,
	...
	isMap = function(thing) {
		// See: https://github.com/mobxjs/mobx/blob/5.6.0/src/utils/utils.ts#L145
...
mobx.observable.object = (function(observable_object) {
	// See: https://github.com/mobxjs/mobx/blob/5.6.0/src/api/observable.ts#160
...
var getValueTypeObservable = function(value) {
	return isObservableObject(value) && isObjectPlain(value)
		? $extend in value[$mobx]
			? OT_EXTENDEDOBJECT
			: OT_OBJECT
...
var getValueType = function(value) {
	return isObservable(value)
		? T_OTHER
		: isObject(value) && isObjectPlain(value)
		? $extend in value
			? T_EXTENDPLAINOBJECT
			: T_PLAINOBJECT
		: isArray(value)
...
var defaultReplaceValue = function(oldValue_OT, newValue_T, oldValue, newValue) {
	if (isCompatibleTypes(oldValue_OT, newValue_T, oldValue, newValue)) return oldValue;
		...
		return newValue_T === T_PLAINOBJECT
			? EMPTY_OBJECT
			: newValue_T === T_ARRAY
			? EMPTY_ARRAY
			: newValue_T === T_MAP
			? EMPTY_MAP
			: newValue_T === T_EXTENDPLAINOBJECT
			? newValue[$extend]
			: undefined;
};
...
var patchObservableProp = function(target, property, newValue, replaceValue) {
	...
	if (typeof newValue !== 'object') {
		...
		// newValue type is: null, plain object, observable, array, class object, regexp, browser object, etc.
		...
		if (replacement === undefined) {
			// For: other/unknown objects, null, observables, etc.
			set(target, property, newValue);
			return;
...
var wrapReplaceValue = function(replaceValue, _defaultReplaceValue) {
	if (replaceValue === undefined) return _defaultReplaceValue;
		...
		return function(oldValue_OT, newValue_T, oldValue, newValue) {
